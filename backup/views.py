...
def assay_detail(request, pk):
    ...
    try:
        ...
        #pdb_url = static(f'PDB/{first_epitope.pdbid}.pdb')
        Seqs = Seq_dict(f'static/PDB/{first_epitope.pdbid}.pdb')
        if '|' in first_epitope.Ag_Chain:
            first_epitope.Ag_Chain = first_epitope.Ag_Chain[0]
        Ag_seq = Seqs[first_epitope.Ag_Chain]
        print(first_epitope.MHC_Chain)
        #MHC通常會有兩條chain
        ...
        template_name = 'assay_noContent.html'
